...
3. **Multiprocessing** — the task is split the same way, but across separate processes instead of threads. Each process has its own interpreter and its own GIL, so this is expected to show a real, roughly core-count-proportional speedup.
...
`UPPER_BOUND` (the range searched for primes) and `RUNS` (number of repeated runs) are constants at the top of `main.py`. Adjust `UPPER_BOUND` if the single-threaded baseline runs too quickly or too slowly to clearly show the differences on the machine.

### Expected output

```text
Counting primes up to 200000, using 8 workers where applicable
